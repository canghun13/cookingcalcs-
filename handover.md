...
0. 🚨 **2026-08-03 기준 최우선 상황**: (a) 07-25부터 검색 노출이 95% 붕괴한 상태, (b) 애드센스 재심사 대기 중,
   (c) **sitemap.xml lastmod가 08-02 전면 수정분에 하나도 반영 안 됨 — 이게 즉시 처리할 1순위 작업**.
   신규 페이지 발행보다 이 세 가지가 먼저다. 자세한 내용은 3번 섹션의 `2026-08-03 (15차)` 항목을 볼 것.
1. 이 문서 전체를 먼저 읽는다.
...
### 2026-08-03 (15차): 🚨 검색 노출 95% 붕괴 확인 — 분석 전용 세션(콘텐츠 변경 0건)

> **다음 세션 최우선 확인 항목.** 이번 세션은 사용자가 GSC/GA 자료를 주고 "신규/보강 있으면 정리해"라고 했으나,
> 데이터가 신규 발행을 정당화하지 않아 **분석만 하고 콘텐츠는 손대지 않음.** 이유는 아래.
...
순서가 중요함: **노출이 먼저 붕괴(07-25)하고, 순위 악화는 3일 뒤(07-28)에 따라옴.**
= 단순 순위 하락이 아니라 **Google이 대부분의 쿼리에서 우리 페이지를 아예 후보에서 뺀 것**에 가까움.

**시점 대조 (원인 추정에 필요한 사실만 나열, 단정하지 말 것)**
- 07-11: Coverage에서 색인 9개 → **70개**로 급증(미색인 53→12). 노출 급증은 정확히 이 시점부터 시작.
- 07-11 ~ 07-24 (14일): 노출이 94 → 1,248로 우상향. **CTR은 이 기간 내내 0.0~0.2%.**
- 07-25: 붕괴.
- 같은 시기 애드센스가 "가치가 별로 없는 콘텐츠"로 거절(14차 세션에서 대응).
- **2026년 7월 코어 업데이트는 06-28~07-09에 이미 종료됨** — 07-25 붕괴와 시점이 안 맞음. 코어 업데이트 탓으로 돌리지 말 것.
  (단 Google이 07-09자로 "소규모 코어 업데이트는 공지 없이 상시 롤아웃"이라고 문서를 갱신한 상태라 무명 업데이트 가능성 자체는 배제 못 함)

**가장 설득력 있는 해석(확정 아님)**: 07-11 대량 색인 직후의 **탐색적 노출 부여(허니문) → 2주간 CTR이 사실상 0 → 재평가 후 강등**.
여기에 14차에서 진단된 문제(정적 HTML에 헤더/네비/푸터 0개, 목록 페이지 20단어대, 홈이 "coming soon")가 겹쳤을 가능성이 높음
— 애드센스가 본 것과 검색 크롤러가 본 것이 같은 화면이었음.

**따라서 14차(08-02)의 정적 렌더링 전환이 애드센스뿐 아니라 검색 붕괴의 대응책이기도 함.**
08-02에 배포됐으므로 **재크롤 → 반영까지 2~3주 필요.** 다음 세션에서 할 일은 새 페이지 발행이 아니라
**노출 회복 여부 확인**이다. 회복 신호 없이 콘텐츠만 더 쌓지 말 것.

#### 발견 2 — sitemap.xml lastmod가 08-02 대수술에 전혀 갱신 안 됨 ⚠️ 즉시 조치 대상
102개 전 페이지의 **본문 HTML이 08-02에 바뀌었는데**(헤더/푸터 정적 삽입, 목록 페이지 카드, 프라이버시/컨택 전면 재작성)
sitemap.xml의 lastmod는 여전히 07-13(31개) / 07-16(9) / 07-18(8) / 07-20(10) / 07-24(10) / 07-27(28)에 멈춰 있음.
**08-02 날짜가 단 1개도 없음.** 이 문서 7번 섹션에 "구글이 크롤 스케줄링에 실제로 쓰는 유일한 필드"라고 적어놓은 바로 그 필드임.
노출 붕괴에서 회복하려면 재크롤이 최우선인데, 정작 "바뀌었다"는 신호를 안 보내고 있는 상태.
→ **다음 세션 1순위 작업: 전 URL lastmod를 배포일로 갱신 후 push + GSC에 sitemap 재제출(사용자 작업).**

#### 발견 3 — GA \"Organic Search\"의 97%는 구글이 아님 (프레이밍 교정)
...
- 지금까지 세션마다 "GSC는 0인데 GA는 는다"를 낙관 근거로 써왔는데, 정확히 말하면
  **"구글에서는 거의 못 벌고 있고, 그나마 있던 구글 노출마저 07-25에 사라졌다"**가 맞는 서술임. 다음 세션은 이 프레이밍으로 시작할 것.
...
- 원인 후보: (a) 평균순위를 끌어올리는 게 노출 1~2회짜리 쿼리라 실제로는 1페이지가 아님(기존 세션들의 해석), (b) 상위 쿼리 상당수가
  **워드프라블럼(숙제)**이라 원래 클릭이 안 남 — "you purchase beef at $6 per pound…"가 노출 1위(54회, 6.39위, 클릭 0),
  (c) AI Overview 흡수.
- **수익화 결론**: 페이지를 96번째로 늘리는 것보다 **이미 노출 400회 이상인데 50~85위에 묶여 있는 페이지**를 1페이지로 올리는 게
  유일하게 의미 있는 레버임. 해당 페이지: `tablespoon-to-teaspoon-guide`(498회/67.5위), `cups-to-grams-guide`(475/55.8),
  `how-long-to-cook-turkey-breast`(414/84.0), `how-long-to-cook-lamb-chops`(359/75.7), 홈(654/68.8).
  단 **노출 회복 전에는 이 작업도 효과 측정이 불가능**하므로 08-02 배포 반영을 먼저 기다릴 것.
