# CookingCalcs 현황 및 인수인계 (2026-09-14 기준)
...
### 2026-08-31 (26차): 주간 점검 — Bing 계속 성장, 버터 질문형 페이지 신설

> 데이터 기준일: GSC/Coverage 08-31, Bing 08-30, GA4 28일.
> (25차·24차는 미래 날짜로 기록돼 있으나 실제 작업 순서는 이 항목이 최신)

#### Bing 성장 계속 (전주 대비)
| | 08-24 | 08-31 |
|---|---|---|
| Bing 노출 | 약 3,700 | **약 5,560** |
| Bing 클릭 | 55 | **79** |
| 구글 노출/클릭 | 9,733 / 5 | 10,034 / **5** |

- 구글은 3개월 노출 10,034에 클릭 5, 평균 46위로 **완전 정체**. 최근 일별 순위 72~82위.
- **Bing 최고 CTR**: `average-cost-of-a-home-cooked-meal` 8노출 5클릭 **62.5%**(2.13위),
  `complete-baking-conversion-guide` 10노출 2클릭 **20%**(3.90위),
  `how-to-reduce-a-recipe` 15노출 2클릭 13.33%, `oven-temp-converter` 11.11%.
- GA4: Organic 397세션(전 370), Direct 288. **총수익 0 — 5개월 연속.**

#### ★ 이번 세션 핵심 발견 — 질문형 페이지가 툴 페이지보다 Bing 순위가 좋다
자사 데이터로 확인된 패턴:
| 페이지 유형 | 예시 | 노출 | Bing 평균순위 |
|---|---|---|---|
| 질문형 블로그 | how-to-substitute-egg-sizes | 510 | **4.41** |
| 질문형 블로그 | how-many-eggs-in-a-cup | 255 | **4.90** |
| 질문형 블로그 | how-many-cups-in-a-pound-of-sugar | 215 | **4.71** |
| 툴 | egg-converter | 998 | 4.85 |
| 툴 | **butter-converter** | **1,243** | **6.99 · CTR 0.32%(사이트 최저)** |

**`how-many-X-in-a-Y` 질문형 패턴이 사이트에 7개 있는데 버터만 비어 있었다.** 그런데 Bing
버터 키워드는 거의 전부 그 형태이고(스틱↔컵 합계 약 113노출), butter-converter가 1,243노출을
받으면서 CTR 0.32%로 새고 있었다. → **빈 슬롯을 채우는 작업**으로 판단.

#### 산출물 — `blog/how-many-cups-in-a-stick-of-butter.html` (1,083단어)
- **경쟁도**: head term 포화(Land O'Lakes·KitchenAid·Cabot·Minerva 등 브랜드 + thecalculatorsite
  + WebstaurantStore·sweetsandthankyou·atlanticse·honokage, 10곳+). 전부 같은 표만 제공.
  단 **새 니치 진입이 아니라 이미 받고 있는 노출에 맞는 페이지 형태를 갖추는 작업**이라 진행.
- **차별화**: (1) **래퍼 눈금으로 안 떨어지는 ⅓컵·⅔컵을 정면으로 다룸** — 나머지가 정확히
  1tsp/2tsp라는 점(1tbsp=3tsp이므로 반올림 아님)까지 설명. 쿠키·퀵브레드에 자주 나오는 양인데
  경쟁 대부분이 표만 주고 넘어감. (2) **역방향(컵→스틱) 표 별도 배치** — Bing 역방향 쿼리만
  22노출. (3) 250g 유럽 블록 = 2.21스틱(9.2% 부족). (4) 녹이기 전 계량.
- **자사 중복 회피**: 기존 3개와 역할 분리(툴 / 계량방법 903단어 / 허브). 사이트에 이미 동일
  3층 선례 있음(cups-to-grams 툴 + how-many-grams-in-a-cup-of-sugar 질문형 + 베이킹 허브).
  수치는 기존 `how-to-measure-butter` 표를 그대로 재사용, ⅓·⅔컵만 227g 기준 신규 산출.
- 산술 전부 python 검증.

#### 구글 색인 (제공 자료 기준, 변화 없음)
'발견됨-미색인' **29개** 유지, 최종 크롤링 전부 `1970-01-01`(미크롤).
'크롤링됨-미색인'은 `complete-baking-conversion-guide` 1개(07-19 크롤) — 이 페이지는
**Bing에서는 3.90위·CTR 20%로 사이트 최상위권**이다. 채널별 평가가 정반대인 사례.
**코드로 고칠 게 없는 항목이며 발행 속도를 늦추는 근거로 쓰지 말 것**(0-2 참고).

#### 신규 후보 판정
| 후보 | 판정 | 근거 |
|---|---|---|
| 버터 스틱↔컵 질문형 | **채택** | 사이트 패턴의 빈 슬롯, Bing 최대 누수 지점 |
| 버터 추가 블로그(계량법·대체) | 기각(자사중복) | `how-to-measure-butter`·`baking-substitutions`가 이미 커버 |

#### 사이트 규모
툴 31 / 블로그 75→**76** / 가이드 14 / sitemap 127→**128**.

#### 다음 세션 확인
- 이 페이지가 Bing에서 butter-converter(6.99위)보다 좋은 순위를 잡는지 확인.
...
### 2026-09-14 (25차): 신규 클러스터 — 식사 타이밍 역산 스케줄러 (툴1+블로그1+가이드1)

#### 경쟁도 판정 (후보 3건 → 2건 기각)
| 후보 | 판정 | 근거 |
|---|---|---|
| 치즈 환산(블록↔슈레드↔컵) | 기각 | **chefsolver**(치즈별 밀도 프로파일 보유 전용 사이트)·challengeanswer·handychefdom·recipes.net·eathealthy365·newmarketdairy·Cabot 등 7곳+. **우리가 쓸 차별화(치즈별·형태별 구분)를 전용 계산기가 이미 선점** |
| 마리네이드 시간 | 기각 | **miniwebtool**(대형 계산기팜) 전용 계산기 + titangrillers 단백질별 페이지 + cupstogramscalculator. **"하한과 상한 구간" 차별화까지 이미 선점** |
| **식사 타이밍 역산** | **채택** | 계산기팜 0개 |

#### ★ 채택 근거 — "웹 도구로 존재하지 않는다"는 직접 증거
경쟁자가 푸드블로그(thespeckledpalate·lifeasastrawberry·marysprintables), **엑셀 템플릿**(contextures),
**유료 앱**(timetoplate SaaS·Easy Cooking Timer iOS), 단일 요리 전용(icalculator)뿐.
결정적으로 **mealie GitHub에 "스프레드시트로 하고 있으니 기능을 만들어달라"는 요청이 남아 있음** —
수요는 있는데 웹 도구가 없다는 증거.

**우리가 이겨온 패턴과 동일한 형태**: 단순 변환이 아니라 **다중입력 스케줄링/판단 문제**.
21차 채소 스태거링·24차 오븐 공유와 같은 계열이고, 그 둘의 상위 개념에 해당하는 캡스톤.
→ **다음 세션 참고: 계산기팜은 "단일 공식 + 고볼륨"만 만든다. 우리 승산은 스케줄링·판단·
교차 클러스터 문제에 있다. 후보를 이 형태로 찾을 것.**

#### 차별화 — 경쟁이 구조적으로 못 하는 것
1. **전원이 "거꾸로 계산하라"는 산문에서 멈추고 실제 시각을 계산해주지 않는다.**
   → `시작시각 = 서빙시각 − 레스팅 − 조리 − 프렙` 구현, 요리별 이벤트를 정렬된 단일 타임라인으로 병합.
2. **오븐 충돌 자동 검출.** marysprintables가 "한 오븐에 가능한지 즉시 알게 된다"고 쓰지만
   계산은 안 해줌 → 오븐 사용 구간 겹침 + 온도차 50°F 초과 시 경고, 50°F 이하면 24차 계산기로 연결.
3. **레스팅 창을 "빈 오븐 자원"으로 제시하고 자동 안내.** 레스팅을 죽은 시간으로 취급하는 것이
   사이드가 식는 원인이라는 관점.
4. 프렙을 별도 블록으로 스케줄(대부분의 플랜이 빠뜨림), 크라우딩 보정·버퍼 15-30분 명시.

#### 산출물
- `tools/meal-timeline-planner.html` (938단어) — 서빙시각+요리별 프렙/조리/레스팅 → 역산 타임라인
- `blog/how-to-time-a-meal.html` (1,130단어) — 방법론 + 3요리 워크드 예제 + 실패 유형표
- `guides/complete-meal-timing-guide.html` (958단어) — 허브

가이드 고유값: **3개 제약(시계·오븐·각 요리의 대기 내성)으로 조직**하고, **홀드 시간표**
(45분+ 로스트 / 30분 캐서롤 / 10분 크리스피 / 즉시 서빙 녹색채소)를 아래에서 위로 읽으면
조리 순서가 된다는 관점 + 6단계 프로세스 + 증상별 진단표.

#### 정합성 검증
계산기와 본문 워크드 예제 일치 node 검증(6:30pm 서빙, prep15/cook80/rest20 → 오븐 4:50 투입,
6:10 꺼냄, 프렙 4:35 시작). 오븐 충돌 판정 확인(닭 400°F 4:50~6:10 vs 감자 425°F 5:45~6:30
겹침, 온도차 25°F → 절충 안내). 크라우딩 10/15%·절충 임계 50°F를 24차와 동일하게 사용.

#### 사이트 규모
툴 30→**31**, 블로그 74→**75**, 가이드 13→**14**, sitemap 124→**127**.
