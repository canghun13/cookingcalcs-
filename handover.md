...
0-2. 🚨 **기본 전략: 공격적 확장(신규 클러스터 발행). 이건 바뀌지 않는다**
   - **Bing이 주력 채널이다.** 09-13 기준 Bing 노출 16,809·클릭 206·CTR 1.23%·평균 약 6위 /
     구글 노출 10,465·클릭 **5**·평균 49위. 구글은 완전 정체이고 Bing만 매주 성장 중.
   - 🚨🚨 **CTR을 가르는 것은 순위가 아니라 "답이 한 줄로 끝나는가"다(30차 실측).**
     ...
     `tablespoon-to-teaspoon-guide`는 1,212노출에 클릭 1개, `oats`·`rice`·`tablespoon-vs-teaspoon`은
     수백 노출에 클릭 0. 답이 "3", "½컵", "90g"이면 Bing이 직접 답해서 클릭이 오지 않는다.
   - 🚨 따라서 **① 단순 단위변환 페이지의 CTR을 올리려는 시도를 하지 말 것**(구조적으로 불가능.
     28차가 시도해 순위만 6.83→6.18로 올리고 클릭은 1개 그대로였다),
     **② 신규는 "답이 한 줄로 안 끝나는 주제"에만 발행할 것**(판단·비교·절차·다단계 계산).
     21~29차 클러스터가 정확히 그 형태이므로 **전략 자체는 이미 맞다.** 상세는 3번 섹션 30차.
   - 후보를 정할 때 **missvickie·agentcalc·calckitchen·crunchmilk(회피리스트)를 먼저 검색**할 것.
     missvickie가 콩(27차)·소금(29차)·베이킹파우더(30차) 3주 연속으로 우리 후보를 선점했다.
   ...
   - **총수익 8개월 연속 0**(31차 기준). 애드센스 심사 중이며 CMP(유럽 규정 메시지)는 게시 완료.
...
### 2026-09-21 (31차): 주간 점검 — 30차 결론 재확인, 판단형 신규가 Bing에서 먹히기 시작. 배치쿡 냉동 계획 신설

> 데이터: GSC/Coverage 09-21, Bing 09-21, GA4 28일.

#### Bing 성장 계속, 구글 정체
| | 09-06 | 09-13 | 09-21 |
|---|---|---|---|
| Bing 노출 | 11,000 | 16,809 | **22,741** |
| Bing 클릭 | 141 | 206 | **257** |
| 구글 노출/클릭 | 10,291/5 | 10,465/5 | **10,594/5** |

GA4 Organic 548세션(전 517), Direct 590. **총수익 0 — 8개월 연속.** click 이벤트 5건.
**CN 287명·SG 202명으로 봇 의심 트래픽 계속 증가**(GSC 노출은 각 2건·46건뿐).

#### ★ 30차 결론 재확인 — 단순 변환은 노출이 늘수록 CTR만 떨어진다
| 페이지 | 09-13 | 09-21 |
|---|---|---|
| tablespoon-to-teaspoon-guide | 1,212노출 / 1클릭 / 0.08% | **2,004노출 / 1클릭 / 0.05%** |
| butter-converter | 4,489 / 13 / 0.29% | 6,010 / 18 / 0.30% |
| how-to-substitute-egg-sizes | 1,419 / 65 / 4.58% | **1,780 / 83 / 4.66%** |
| average-cost-of-a-home-cooked-meal | 126 / 13 / 10.32% | 141 / 13 / 9.22% |

tablespoon-to-teaspoon은 노출이 65% 늘었는데 클릭은 1개 그대로. **30차 결론이 한 주 더 버텼다.**

#### ★★ 21~29차 판단형 신규가 Bing에서 먹히기 시작했다 (전략 실증)
| 페이지 | 노출 | 클릭 | CTR | 평균순위 |
|---|---|---|---|---|
| **altitude-cooking-calculator** | 5 | **3** | **60%** | 4.60 |
| grill-temperature-calculator | 11 | 0 | — | 4.36 |
| cooking-time-calculator | 9 | 0 | — | 5.22 |
| what-size-pot-do-i-need(29차) | 3 | 0 | — | 6.33 |
| pressure-cooker-converter | 10 | 0 | — | 17.70 |
| reheating-calculator | 2 | 0 | — | 10.00 |

노출은 아직 작지만 **altitude가 5노출 중 3클릭**. 구글이 크롤도 안 한 페이지들이 Bing에서 잡히고 있다.
**→ 21~29차의 "판단·스케줄링형 클러스터" 전략이 옳았다는 첫 실측 증거.** 계속 같은 형태로 발행할 것.

#### 산출물 — 배치쿡 냉동 계획 (meal-prep 클러스터 확장)
- `tools/batch-cooking-planner.html` (1,087단어) — 가구원수·주당횟수·기간·음식종류·레시피인분 →
  레시피 배율·용기 수·총 인분·소진 기한. 보관기간 초과·4배 초과 시 경고
- `blog/how-long-do-freezer-meals-last.html` (1,250단어)

**선정 근거**: meal-prep은 **CTR 최고 영역**(average-cost 9.22%)인데 클러스터 4개가 전부 "비용" 중심이고
**냉동 분량·보관기간·용기 수는 미커버**(container 0~1회, "how long" 0~1회). 냉동 보관기간 전용 페이지 0건.
새 허브 없이 **기존 클러스터 확장**으로 처리.

**경쟁**: melissaknorris·soupercubes·frugalfamily·kitchengearuk·percolatekitchen·littlehouseliving(블로그),
mealthinker(AI 밀플래너), barnardos(NHS계열). **계산기팜 0개.** 전부 산문 가이드이고
"몇 배로 만들어 몇 통에 나누고 언제까지 먹나"를 계산하는 곳 없음.

**차별화**: (1) **"냄비 크기가 아니라 먹는 빈도에서 역산하라"** — 흔한 실수(냄비를 채워 만들고
석 달 뒤 절반 버림)를 정면으로, (2) **"안전과 품질은 다른 시계"** — 0°F 무기한 안전,
차트 기간은 품질 기한. 시중 차트가 구분 안 해서 안전한 음식을 버리게 만든다는 관점,
(3) 액체 요리가 잘 얼는 메커니즘, (4) 해동방식별 재냉동 가능 여부 표,
(5) **29차 냄비 용량 계산기와 연결**.

**정합성**: 툴·블로그 보관기간 일치(12주=3개월, 16주=4개월 가금류, 8주=2개월 밥·파스타).
2시간 룰·165°F는 기존 페이지와 동일값. 워크드 예제 node 검증.
**작성 중 오류 1건 발견·수정**: "칠리는 8주 보관"으로 읽히는 문장이 표(2~3개월)와 어긋나 정정.

#### 신규 후보 판정
| 후보 | 판정 | 근거 |
|---|---|---|
| prime rib 조리시간 | **기각** | **missvickie(회피리스트) 계산기 2개 + easytemperature(회피리스트)** + quickcooktime·meatcookingtimecalculator 전용 계산기. 무게·등급·뼈·시작온도·도네스·시어방식·레스팅까지 선점. 30차 경고대로 회피리스트 먼저 검색해서 걸러냄 |
| **배치쿡 냉동 계획** | **채택** | 위 참고 |

**missvickie 선점 이력: 콩(27차)·소금(29차)·베이킹파우더(30차)·prime rib(31차) — 4주 연속.**

#### 구글 색인 (제공 자료 기준)
'발견됨-미색인' **46개**(전주 44). 최종 크롤링 전부 `1970-01-01`. '크롤링됨-미색인'은
`complete-baking-conversion-guide` 1개 — **Bing에서는 4.43위·CTR 7.89%**. 채널별 정반대 평가 유지.
**코드로 고칠 게 없으며 발행 속도를 늦추는 근거로 쓰지 말 것**(0-2 참고).

#### 사이트 규모
툴 33→**34**, 블로그 79→**80**, 가이드 15, sitemap 134→**136**.

### 2026-09-14 (30차): 주간 점검 — ★★★ CTR을 가르는 것은 순위가 아니라 "답의 형태"라는 발견. 신규 0건

> 데이터: GSC/Coverage 09-14, Bing 09-13, GA4 28일. **신규 후보 2건 모두 기각 → 콘텐츠 변경 0건.**
> 이번 세션의 가치는 발행이 아니라 아래 발견에 있다. 다음 세션은 이 항목을 반드시 먼저 읽을 것.

#### Bing 성장은 계속
| | 08-31 | 09-06 | 09-13 |
|---|---|---|---|
| Bing 노출 | 5,560 | 11,000 | **16,809** |
| Bing 클릭 | 79 | 141 | **206** |
| 구글 노출/클릭 | 10,034/5 | 10,291/5 | **10,465/5** |

구글은 3개월 클릭 5로 고정, 평균 49위, 최근 일별 노출 23~67로 급감 유지. GA4 Organic 517세션.
**총수익 0 — 7개월 연속.** click 이벤트 8건. CN 256명·SG 64명(봇 의심 계속).

#### ★★★ 28차 보강은 순위를 올렸지만 클릭은 못 늘렸다
`tablespoon-to-teaspoon-guide`를 질문형으로 보강한 결과:
| | 09-06(보강 전) | 09-13(보강 후) |
|---|---|---|
| Bing 노출 | 392 | **1,212** |
| Bing 평균순위 | 6.83 | **6.18** ← 개선 |
| 클릭 | 1 | **1** ← 변화 없음 |
| CTR | 0.26% | **0.08%** ← 악화 |

**순위는 올랐고 노출은 3배가 됐는데 클릭은 1개 그대로다.** 26차 가설("질문형이 Bing 순위를 올린다")의
순위 부분은 맞았고, **"순위가 오르면 클릭이 온다"는 전제가 틀렸다.**

#### ★★★ 진짜 변수는 "답이 한 줄로 끝나는가"다 (1클릭당 노출 실측)
| 페이지 | 노출 | 클릭 | CTR | **1클릭당 노출** |
|---|---|---|---|---|
| tablespoon-to-teaspoon-guide | 1,212 | 1 | 0.08% | **1,212** |
| butter-converter | 4,489 | 13 | 0.29% | **345** |
| how-to-measure-butter | 1,071 | 5 | 0.47% | 214 |
| liquid-converter | 2,364 | 19 | 0.80% | 124 |
| grams-in-a-cup-of-oats | 322 | **0** | 0% | ∞ |
| grams-in-a-cup-of-rice | 218 | **0** | 0% | ∞ |
| tablespoon-vs-teaspoon | 217 | **0** | 0% | ∞ |
| — | | | | |
| how-to-substitute-egg-sizes | 1,419 | **65** | 4.58% | **22** |
| how-long-to-cook-ribs | 16 | 2 | 12.50% | **8** |
| **average-cost-of-a-home-cooked-meal** | **126** | **13** | **10.32%** | **10** |
| complete-baking-conversion-guide | 67 | 6 | 8.96% | 11 |
| herb-converter | 22 | 2 | 9.09% | 11 |
| how-to-reduce-a-recipe-by-half | 146 | 5 | 3.42% | 29 |

**`butter-converter`는 4,489노출로 13클릭, `average-cost`는 126노출로 13클릭 — 노출 대비 효율이 35.6배 차이다.**

가르는 기준이 명확하다. **답이 단일 숫자로 끝나는 쿼리는 순위가 몇 위든 클릭이 오지 않는다.**
"how many teaspoons in a tablespoon"의 답은 3이고, "1 stick of butter is how many cups"의 답은 ½컵이고,
"1 cup oats in grams"의 답은 90g이다. **Bing이 검색결과에서 직접 답하므로 클릭할 이유가 없다.**
반면 계란 사이즈 치환·집밥 비용·레시피 반감·갈비 조리시간은 표나 절차가 필요해서 클릭이 발생한다.

#### 🚨 수익화 관점의 결론 — 다음 세션이 반드시 지킬 것
1. **단순 단위변환 페이지의 CTR을 올리려는 시도를 하지 말 것.** 구조적으로 불가능하다.
   28차가 그걸 시도해서 순위만 올리고 클릭은 못 얻었다. 노출 숫자가 커도 매출로 이어지지 않는다.
2. **"답이 한 줄로 안 끝나는 주제"에만 신규를 발행할 것** — 판단·비교·절차·다단계 계산.
   **우리가 21~29차에 만든 클러스터(채소 스태거링·오븐 공유·식사 타이밍·딥프라잉·냄비 용량)가
   정확히 그 형태다. 전략은 이미 맞다.** 다만 아직 색인·노출이 안 붙었을 뿐이다.
3. **최고 효율 주제는 비용/예산이다**(`average-cost` 1클릭당 10노출, CTR 10.32%). 노출이 126뿐인데
   클릭 13개. 이 클러스터를 키우는 게 수익화에 가장 직접적이다 — 단 아래 기각 사유를 먼저 볼 것.

#### 26차 신규 페이지, 드디어 Bing 노출 시작
`how-many-cups-in-a-stick-of-butter`(08-31 발행)가 **20노출·평균 5.70위·클릭 0**.
순위는 `butter-converter`(6.46위)보다 좋다 → 질문형이 순위를 올린다는 건 재확인.
그러나 클릭 0 → 위의 결론과 일치. **버터 스틱↔컵은 답이 한 줄이라 애초에 클릭이 안 오는 주제였다.**

#### 신규 후보 판정 — 2건 모두 기각
| 후보 | 판정 | 근거 |
|---|---|---|
| 집밥 vs 외식·배달 비용 비교 | **기각(자사중복)** | `average-cost-of-a-home-cooked-meal`(1,133단어)에 이미 "Home-Cooked vs. Restaurant: The Real Gap" 섹션 존재. 배달앱 비용은 미커버이나 금융 미디어 경쟁 영역이고 계산기 사이트 정체성에서 벗어남 |
| 베이킹파우더 ↔ 베이킹소다 치환 | **기각(외부선점)** | **missvickie(회피리스트)가 전용 계산기 보유**, 그것도 레시피 스타일별·배치 스케일링·밀가루 부하·소금까지 반영한 정교한 버전. 내가 만들 차별화를 전부 선점. Healthline·McCormick·The Mom 100·Bakers Authority·thelivinglook(2페이지)까지 포함 |

**→ 27차에 적어둔 "회피리스트 도메인이 니치를 계속 확장 중이니 기각 이력이 있어도 재검색할 것"이
또 적중했다.** missvickie는 콩(27차)·소금(29차)·베이킹파우더(30차) 3주 연속으로 우리 후보를 선점했다.
**다음 세션은 후보를 정할 때 missvickie·agentcalc·calckitchen·crunchmilk를 먼저 검색해볼 것.**

#### 구글 색인 (제공 자료 기준)
'발견됨-미색인' **44개**(전주 45, 실질 변화 없음). 최종 크롤링 전부 `1970-01-01`(미크롤).
21~29차 신규가 대부분 이 목록에 포함. '크롤링됨-미색인'은 `complete-baking-conversion-guide` 1개 —
**이 페이지는 Bing에서 4.22위·CTR 8.96%로 최상위권.** 채널별 평가가 정반대인 사례 계속 유지.
**코드로 고칠 게 없으며 발행 속도를 늦추는 근거로 쓰지 말 것**(0-2 참고).

#### 사이트 규모
툴 33 / 블로그 79 / 가이드 15 / sitemap 134 — **변동 없음(신규 0건).**

### 2026-09-28 (29차): 신규 — 냄비 용량/장비 한계 (툴1+블로그1, 스케일링 클러스터 확장)

#### 경쟁도 판정 (후보 2건 기각 후 채택)
| 후보 | 판정 | 근거 |
|---|---|---|
| 소금 종류 변환(코셔/테이블/시솔트) | **기각** | **missvickie·agentcalc(둘 다 회피리스트)**가 전용 계산기 보유. 특히 **agentcalc는 커스텀 밀도 입력·USDA 나트륨 계산·브랜드별 라벨값까지** 담아 내가 쓸 차별화를 전부 선점. saltpepperskillet 계산기, tablespoontoteaspoon 전용 페이지, ATK/Taste of Home/Simply Recipes 포함 7곳+ |
| 냄비 크기 "정면"(제품 선택) | **기각** | prudentreviews·solamexhome(5개 페이지)·cookwareapex·kitchencrafthubs·kitchenwarecw 등 **전부 쿡웨어 판매/리뷰 사이트**. "어떤 냄비를 살까"는 제품 구매 가이드 니치라 정체성 이탈(에어프라이어 용량 기각과 동일 기준) |
| **냄비 용량 "판단"** | **채택** | 각도를 **"내가 가진 냄비로 이 레시피가 되나 / 몇 배까지 되나"**로 전환. 제품 선택이 아니라 판단 문제 |

**→ 27차 교훈이 또 확인됨**: 단일 변환(소금)은 이미 계산기팜이 먹었고, 다중입력 판단 문제만 비어 있었다.
그리고 같은 주제라도 **각도를 제품→판단으로 틀면 열린다**는 게 이번의 추가 학습.

#### 자사 갭
- 냄비 용량·끓어넘침·헤드스페이스 전용 커버리지 **0건**(딥프라잉의 기름 절반 규칙만 존재)
- **`complete-recipe-scaling-guide`(1,024단어)가 장비 한계를 전혀 안 다룸: pot 0회, equipment 0회**
→ 스케일링 클러스터의 명백한 빈 곳. **새 허브를 만들지 않고 기존 클러스터를 확장**하는 형태로 처리.

#### 차별화
1. 경쟁이 전부 "제품 구매 가이드"인데 우리는 **"이미 가진 냄비로 되나"**를 계산. 재료 배율
   (`recipe-multiplier`)과 장비 한계를 잇는 페이지가 없었음.
2. **"표시 용량은 림까지 잰 값이라 실제로는 ⅔만 쓴다"**를 전면 배치. 8쿼트 = 실사용 5⅓쿼트.
   2배 레시피가 안 들어가는 이유가 이것.
3. **요리별 충전선 차등**(스톡 ¾ / 수프·스튜 ⅔ / 파스타는 물 기준 ⅔ / 튀김 ½).
   튀김 ½는 기존 `how-much-oil-for-deep-frying`과 동일값 재사용.
4. **파스타는 완성 부피가 아니라 물로 크기가 정해진다**는 예외를 별도 섹션으로.
   1lb = 물 4쿼트 → 6쿼트 냄비 최소. **4인분 요리인데 10인분 수프와 같은 냄비**라는 역설.
5. **"넘치는 건 액체가 아니라 거품"** — 반쯤 빈 냄비가 넘치는 이유.
6. 2배 스케일 시 필요 냄비 크기 표.

#### 산출물
- `tools/pot-size-calculator.html` (968단어) — 냄비 크기+요리 종류(+레시피 인분) →
  안전 충전량·수용 인분·최대 배율. 파스타/튀김은 별도 로직.
- `blog/what-size-pot-do-i-need.html` (1,202단어)

#### 정합성 검증
본문 표의 모든 산술 node 검증 — 냄비 표 3~20쿼트 안전충전량·서빙수 전부 일치,
FAQ "6인분 수프 = 9컵 = 2.25쿼트" 일치, 파스타 "1lb=4쿼트 물 → 6쿼트 최소" 일치,
2배 표 전부 정합. **1서빙 = 1.5컵 = 0.375쿼트 기준을 본문에 명시**하고
`what-is-a-serving-size`로 링크. 튀김 1:6 비율도 딥프라잉 페이지와 동일값.

#### 사이트 규모
툴 32→**33**, 블로그 78→**79**, 가이드 15(변동 없음), sitemap 132→**134**.

### 2026-09-07 (28차): 주간 점검 — Bing 클릭 141로 급성장, tbsp/tsp 페이지 질문형 보강

> 데이터: GSC/Coverage 09-07, Bing 09-06, GA4 28일.
> (24~27차가 미래 날짜로 기록돼 있으나 실제 작업 순서상 이 항목이 최신)

#### Bing 급성장 계속
| | 08-24 | 08-31 | 09-06 |
|---|---|---|---|
| Bing 노출 | 3,700 | 5,560 | **약 11,000** |
| Bing 클릭 | 55 | 79 | **141** |
| 구글 노출/클릭 | 9,733/5 | 10,034/5 | **10,291/5** |

- 구글은 3개월 클릭 5로 **완전 정체**(평균 46위, 최근 일별 65~80위). 노출도 9월 들어 25~33/일로 급감.
- **Bing 주력**: butter-converter 3,013노출(전주 1,243의 2.4배) / liquid 1,983 /
  egg-converter 1,477·28클릭·1.9% / **how-to-substitute-egg-sizes 1,027·40클릭·3.89%(최대 클릭원)**
- **Bing 최고 CTR**: `average-cost-of-a-home-cooked-meal` 24노출 7클릭 **29.17%**(2.83위),
  `complete-baking-conversion-guide` 44노출 4클릭 **9.09%**(3.68위), herb-converter 9.09%,
  cooking-measurement-guide 9.52%
- GA4: Organic 439세션(전 397). **총수익 0 — 6개월 연속.**

#### 26차 신규 페이지 상태
`blog/how-many-cups-in-a-stick-of-butter.html`(08-31 발행)은 **Bing 노출 아직 0**.
계란 허브가 1주 만에 158노출을 받은 것과 대비되나, 발행 후 6일이라 판단하기 이름.
**다음 세션에서 반드시 재확인할 것** — 잡히면 26차의 "질문형 페이지" 가설이 검증되고,
안 잡히면 가설을 재검토해야 함.

#### 산출물 — `blog/tablespoon-to-teaspoon-guide.html` 보강 (1,149→1,451단어)
**근거**: 이 페이지가 392노출·클릭 1·**CTR 0.26%**·6.83위로 Bing 최대 누수 지점.
자매 페이지 `tablespoon-vs-teaspoon` 112노출 0클릭까지 합치면 **504노출에 클릭 1**.
26차 handover가 "다음 후보: 182노출 클릭0"로 지목한 페이지가 2배로 늘어난 상태였음.

**신규가 아니라 보강을 택한 이유(중요)**: 26차 버터 케이스와 달리 여기는 신규 발행이
명백한 자기잠식이었음 —
- 기존 guide(1,149단어)가 이미 변환 가이드이고 차트에 ⅛·¼·½ tbsp까지 포함
- `tablespoon-vs-teaspoon`(915단어)도 같은 3 tsp = 1 tbsp를 다룸
- 외부 경쟁도 MasterClass·The Kitchn·Skillshare·YesChef·simple-veganista·
  **thecalculatorsite** 등 9곳+이고 **분수 표까지 이미 전부 커버**
→ 세 번째 페이지 대신 **기존 페이지를 질문형으로 재포지셔닝**. 슬러그 유지로 색인·링크 손실 없음.

**보강 내용**: (1) title/H1/JSON-LD headline을 질문형으로,
(2) 답 선행 리드 문단, (3) **분수/소수 표 신설** — ⅛~2 tbsp를 분수·소수·tsp·측정법 4열로,
**"3분의 1이 오히려 깔끔한 분수"**라는 관점 추가(tbsp가 3 tsp라서 ⅓·⅔가 정확히 1·2 tsp에
떨어지고 ¼이 오히려 지저분함 — 경쟁 콘텐츠에 없는 각도), (4) **역방향 표(tsp→tbsp) 신설**.
표의 모든 산술 node 검증 완료. 자매 페이지의 UK/호주 서술과 모순 없음을 확인.

#### 구글 색인 (제공 자료 기준)
**'발견됨-미색인' 29 → 45개로 급증.** 최종 크롤링 전부 `1970-01-01`(미크롤) 유지.
21~27차 신규가 대부분 이 목록에 포함(deep-frying 3종, meal-timeline, oven-sharing 3종 등).
'크롤링됨-미색인'은 `complete-baking-conversion-guide` 1개 — **이 페이지는 Bing에서
3.68위·CTR 9.09%로 최상위권.** 채널별 평가가 정반대인 사례가 계속 유지되고 있음.
**코드로 고칠 게 없는 항목이며 발행 속도를 늦추는 근거로 쓰지 말 것**(0-2 참고).

#### 신규 후보 판정
| 후보 | 판정 | 근거 |
|---|---|---|
| `how-many-teaspoons-in-a-tablespoon` 신규 | **기각(자사중복)** | 기존 2개 페이지가 같은 내용. 외부도 9곳+ 포화이고 분수까지 커버 → 보강으로 전환 |
| 계란 사이즈 차트 신규 | 기각(자사중복) | `how-to-substitute-egg-sizes` 등 5개 페이지가 이미 촘촘 |

#### 사이트 규모
툴 32 / 블로그 78 / 가이드 15 / sitemap 132 — **변동 없음(보강만)**.

#### 다음 세션 확인
1. **`how-many-cups-in-a-stick-of-butter`가 Bing에 잡혔는지** — 26차 가설의 검증 지점.
2. 이번 보강 후 `tablespoon-to-teaspoon-guide`의 CTR이 0.26%에서 움직이는지.
   움직이면 **"툴/가이드는 있는데 타이틀이 질문형이 아닌 페이지"를 찾아 보강하는 패턴**이
   신규 발행과 병행할 수 있는 두 번째 수단으로 확립됨.
   현재 같은 조건 후보: `how-many-grams-in-a-cup-of-oats` 260노출 **0클릭** 5.30위,
   `how-many-grams-in-a-cup-of-rice` 152노출 **0클릭** 7.28위 — 둘 다 이미 질문형 타이틀인데
   클릭이 0이라 **다른 원인일 가능성**이 있음. 원인 분석부터 할 것.

### 2026-09-21 (27차): 신규 클러스터 — 딥프라잉 (툴1+블로그2+가이드1)

#### 경쟁도 판정 (후보 2건 → 1건 기각)
| 후보 | 판정 | 근거 |
|---|---|---|
| 콩/레귬 침지·조리 | **기각** | **missvickie(회피리스트)가 2주 전 Bean Soaking Time Calculator를 만들었고**, 내가 쓰려던 차별화(고도·압력솥·오래된 콩·물온도·소금브라인·수율)를 **전부 선점**. healwithfood도 30종 침지 차트 보유 |
| **딥프라잉** | **채택** | 경쟁자가 푸드블로그 + **.edu Extension(Oklahoma State)** + Wikipedia + 딥프라이어 **특허 문서**. 계산기팜 0개 |

#### ★ 차별화 — 25차 교훈("스케줄링·판단 문제를 찾아라")을 그대로 적용
**전원이 "팬을 과밀하게 채우지 마라"는 산문 조언만 하고 얼마나 넣어야 하는지 아무도 계산해주지
않는다.** 21차 채소 스태거링 · 24차 오븐 공유 · 25차 식사 타이밍과 정확히 같은 형태.

근거 있는 수치를 확보한 게 이번 건의 강점:
- **음식:기름 = 1:6 부피비**가 회복시간을 최소화한다는 실증(Wikipedia 인용) → 기름 6컵이면
  배치당 약 1컵. **대부분이 예상하는 것보다 훨씬 적고, 그 격차가 기름진 결과의 원인**이라는 게
  이 페이지의 핵심 메시지.
- **Oklahoma State Extension: 상업용 배치 프라이어 풀로드 투입 시 기름 온도 86~104°F 하락**,
  냉동은 더 큼. → 가정용은 훨씬 작지만 메커니즘 동일. **상업용 데이터임을 본문에 명시**해
  가정용에 그대로 적용하는 과장을 회피함.
- 320~375°F 범위, 고도 보정 -3°F/1000ft(자사 `altitude-cooking-calculator`와 동일값 재사용).

#### 산출물
- `tools/deep-frying-calculator.html` (1,041단어) — 기름량+음식량+상태 → 배치크기·배치수·
  예상 온도강하·회복 포함 총시간. 과부하 시 경고
- `blog/deep-frying-temperature-chart.html` (1,078단어) — 16개 음식 + **3개 온도 밴드**
  ("내부가 조리돼야 하나, 데우기만 하면 되나"로 밴드가 갈린다는 관점) + 감자 두 번 튀기는 이유
- `blog/how-much-oil-for-deep-frying.html` (1,211단어) — 냄비크기별 기름량,
  **"절반만" 규칙이 안전 규칙인 이유**(음식 투입 시 부피 상승 + 수분 거품 → 화구로 넘침),
  기름 종류·재사용 판단 기준·폐기
- `guides/complete-deep-frying-guide.html` (1,237단어) — 허브

가이드 고유값: **4단계 의사결정 순서**(기름/냄비 → 배치크기 → 온도 → 익힘 확인)로 조직하고
**"대부분 3번부터 시작해서 1·2번을 의식적으로 결정하지 않기 때문에 애써 맞춘 온도가 유지되지
않는다"**는 관점 + 증상별 진단표(대부분의 행이 배치크기·기름량으로 해결됨을 보여줌).

#### 정합성 검증
계산기 로직과 본문 수치 일치 node 검증(1:6 정확히 맞추면 기준 드롭값 반환, 기름6컵→배치1컵,
8컵→1.33컵, 375−3×5=360°F 전부 본문과 일치). 기존 `altitude-cooking-calculator`의 -3°F/1000ft,
`meat-temperature-guide`의 165°F와 충돌 없음.

#### 사이트 규모
툴 31→**32**, 블로그 76→**78**, 가이드 14→**15**, sitemap 128→**132**.

#### 다음 세션 참고
- **25차 교훈이 다시 유효함을 확인**: 계산기팜은 단일 공식+고볼륨만 만든다. 콩은 이미
  missvickie가 계산기를 만들어 막혔고, 딥프라잉은 "배치 크기"라는 판단 문제라 비어 있었다.
  후보를 **"모두가 산문으로만 조언하는 다중입력 문제"** 형태로 계속 찾을 것.
- 회피리스트 도메인(missvickie·agentcalc·calckitchen·crunchmilk)이 니치를 계속 확장 중이므로
  기각 이력이 있어도 재검색 시 새 계산기가 생겼는지 확인할 것.
