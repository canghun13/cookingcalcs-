...
0-2. 🚨🚨 **2026-08-10 기준 최우선 사항 — 신규 발행보다 먼저 읽을 것**
   - **07-24 / 07-27 발행분 25개 전부, 3개월간 구글 노출 0회.** 여기에 07-18 Guides 3개, 08-03 신규 2개를
     더하면 sitemap 104개 중 **33개(32%)가 노출 0**. 즉 최근 3주간의 발행물은 구글에서 사실상 존재하지 않는다.
   - **사이트 내 800단어 미만 페이지 17개는 예외 없이 전부 노출 0.** (노출0 페이지 평균 818단어 vs
     노출>0 페이지 평균 1,049단어). 4번 섹션의 "800~1200단어" 규칙을 07-24/27 세션이 17번 어겼고,
     그 페이지들이 정확히 안 뜨고 있다. **신규 발행보다 이 17개 보강이 먼저다.** 목록은 3번 섹션 17차 항목.
   - **"공격적 확장(신규 발행)" 기조는 여기서 일시 정지한다.** 발행량이 부족해서 안 되는 게 아니라,
     발행한 게 색인·노출이 안 되고 있는 상태다. 페이지 105번째를 만드는 것보다 안 뜨는 33개를 살리는 게 우선.
...
(3) 800단어 미만 17개 보강(콘텐츠 작업, 신규보다 우선),
