# CookingCalcs 현황 및 인수인계 (2026-08-14 기준)
...
0-2. 🚨 **2026-08-14 기준 진행 상황 — 보강 17개 완료 + 필수 클러스터 신규 2건 발행**
   - **19차(08-14)에서 사용자 지시로 "신규 발행 일시 정지" 기조를 해제**하고, 구조적 빈칸이던
     압력솥·고도·에어프라이어 3툴 + 지원 블로그 3 + 가이드 허브 1을 발행해 **가전 클러스터를 완성**함(툴 27 / 블로그 69 / 가이드 8 / sitemap 111). 상세는 아래 19차 항목.
   - 에어프라이어는 **변환 계산기 축은 포화가 맞지만, 음식별 실측 시간표 축으로 발행 완료**
     (`tools/air-fryer-cooking-times.html`). 처음엔 기각했다가 사용자 지적으로 뒤집은 건이라
     19차 항목의 '교훈' 부분을 반드시 읽을 것.
   - 아래는 18차까지의 상황(계속 유효):
   - **17차(08-10 오전)가 지적한 800단어 미만 17개 페이지, 같은 날 오후 세션(Sonnet 위임)이 전부
     900~1050단어대로 보강 완료함(17/17, 배치 1~5, 커밋 `8753739`~`701c4c0`).** 신규 수치는 사이트 내
     기존 값 재사용 또는 웹서치로 신규 검증한 값만 사용, 자사 중복 확인 후 진행. 상세 내역은 바로 아래
     "2026-08-10 (18차)" 항목 참고.
   - **"공격적 확장(신규 발행)" 기조는 계속 일시 정지 상태.** 07-24/07-27/08-03 발행분(sitemap 104개 중
     33개, 3개월 노출 0)이 이번 보강으로 실제 노출이 붙는지 **다음 GSC 데이터로 반드시 확인부터 할 것** —
     확인 전에 신규를 더 늘리는 건 원인 파악 안 된 상태에서 문제를 키우는 것과 같다.
   - 다음 세션이 최우선으로 할 일: (1) 이번에 보강한 17개 중 몇 개가 다음 GSC export에서 노출이
     발생했는지 대조, (2) 애드센스 재심사 여부 확인(사용자 행동 대기 중, 12번 섹션), (3) GA4 세션
     소스/매체 확인(17차가 제기한 GA4/GSC 40배 격차 미검증 문제, 3번 섹션 17차 "발견 6" 참고).
...
#### 사이트 규모
툴 24 → **27**, 블로그 66 → **69**, 가이드 7 → **8**, sitemap 104 → **111**. 가전 클러스터(툴4+블로그2+가이드1)와 고도 편입까지 포함한 수치.
